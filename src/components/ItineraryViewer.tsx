...
import { TravelItinerary, ItineraryActivity, PreTravelCheck, PlannerPreferences } from '../types';
import {
  Calendar, CheckSquare, Square, PiggyBank, Mail, Share2, Printer, Home,
...
export default function ItineraryViewer({ 
  ...
  // Safely extract IATA codes to configure Skyscanner and Kayak deep paths without 404 errors
  const depIata = departureCity.match(/\(([A-Z]{3})\)/)?.[1];
  const destIata = itinerary.destination.match(/\(([A-Z]{3})\)/)?.[1];

  // Helper to format date for Skyscanner (YYMMDD)
  const formatSkyscannerDate = (dateStr?: string) => {
    if (!dateStr) return '';
    return dateStr.replace(/-/g, '').substring(2);
  };

  // Helper to parse dates from various formats (e.g., "Monday, June 15, 2026", "2026-06-15", "Day 1 (June 15, 2026)") into standard YYYY-MM-DD
  const parseItineraryDateToYMD = (dateStr: string): string | null => {
    if (!dateStr) return null;
    
    // If it's already in YYYY-MM-DD format
    const ymdMatch = dateStr.match(/\b(\d{4})-(\d{2})-(\d{2})\b/);
    if (ymdMatch) {
      return ymdMatch[0];
    }
    
    // Try to clean up strings like "Day 1 (June 15, 2026)" or "Day 1 (2026-06-15)"
    const parenMatch = dateStr.match(/\(([^)]+)\)/);
    if (parenMatch) {
      const parsed = parseItineraryDateToYMD(parenMatch[1]);
      if (parsed) return parsed;
    }

    // Try parsing with Javascript Date parser
    try {
  ...
  // Extract effective travel dates (using datepicker selections or backing into dates recommended/generated by the AI)
  const effectiveStartDate = (() => {
    if (preferences?.hasDates && preferences.startDate) {
      return preferences.startDate;
    }
    if (itinerary.days && itinerary.days.length > 0) {
      const firstDayDateStr = itinerary.days[0].date;
      const parsed = parseItineraryDateToYMD(firstDayDateStr);
      if (parsed) return parsed;
    }
    return null;
  })();
  ...
  const effectiveEndDate = (() => {
    if (preferences?.hasDates && preferences.endDate) {
      return preferences.endDate;
    }
    if (itinerary.days && itinerary.days.length > 0) {
      const lastDayDateStr = itinerary.days[itinerary.days.length - 1].date;
      const parsed = parseItineraryDateToYMD(lastDayDateStr);
      if (parsed) return parsed;
    }
    return null;
  })();

  const hasEffectiveDates = !!(effectiveStartDate && effectiveEndDate);

  const skyscannerUrl = (() => {
    if (hasEffectiveDates && effectiveStartDate && effectiveEndDate) {
      if (depIata && destIata) {
        return `https://www.skyscanner.com/transport/flights/${depIata.toLowerCase()}/${destIata.toLowerCase()}/${formatSkyscannerDate(effectiveStartDate)}/${formatSkyscannerDate(effectiveEndDate)}/`;
      }
      return `https://www.google.com/search?q=Skyscanner+flights+from+${encodeURIComponent(departureCity)}+to+${encodeURIComponent(itinerary.destination)}+on+${effectiveStartDate}+to+${effectiveEndDate}`;
    }
    return (depIata && destIata) 
      ? `https://www.skyscanner.com/transport/flights/${depIata.toLowerCase()}/${destIata.toLowerCase()}/`
      : `https://www.google.com/search?q=Skyscanner+flights+from+${encodeURIComponent(departureCity)}+to+${encodeURIComponent(itinerary.destination)}`;
  })();

  const kayakUrl = (() => {
    if (hasEffectiveDates && effectiveStartDate && effectiveEndDate) {
      if (depIata && destIata) {
        return `https://www.kayak.com/flights/${depIata.toUpperCase()}-${destIata.toUpperCase()}/${effectiveStartDate}/${effectiveEndDate}`;
      }
      return `https://www.google.com/search?q=Kayak+flights+from+${encodeURIComponent(departureCity)}+to+${encodeURIComponent(itinerary.destination)}+on+${effectiveStartDate}+to+${effectiveEndDate}`;
    }
    return (depIata && destIata)
      ? `https://www.kayak.com/flights/${depIata.toUpperCase()}-${destIata.toUpperCase()}`
      : `https://www.google.com/search?q=Kayak+flights+from+${encodeURIComponent(departureCity)}+to+${encodeURIComponent(itinerary.destination)}`;
  })();

  // Google Flights dynamic URL
  const dynamicGoogleFlightsUrl = (() => {
    if (hasEffectiveDates && effectiveStartDate && effectiveEndDate) {
      if (depIata && destIata) {
        return `https://www.google.com/travel/flights?q=Flights%20from%20${depIata}%20to%20${destIata}%20on%20${effectiveStartDate}%20to%20${effectiveEndDate}`;
      }
      return `https://www.google.com/search?q=google+flights+from+${encodeURIComponent(departureCity)}+to+${encodeURIComponent(itinerary.destination)}+departing+${effectiveStartDate}+returning+${effectiveEndDate}`;
    }
    return itinerary.flightDetails?.bookingDeepLink || `https://www.google.com/search?q=google+flights+from+${encodeURIComponent(departureCity)}+to+${encodeURIComponent(itinerary.destination)}`;
  })();

  // Accommodation dynamic URL (Booking / Hotel Search)
  const dynamicAccommodationUrl = (() => {
    const destClean = itinerary.destination || '';
    if (hasEffectiveDates && effectiveStartDate && effectiveEndDate) {
      return `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(destClean)}&checkin=${effectiveStartDate}&checkout=${effectiveEndDate}`;
    }
    return itinerary.budget?.accommodation?.deepLink || `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(destClean)}`;
  })();

  // Transport dynamic URL
  const dynamicTransportUrl = (() => {
    const destClean = itinerary.destination || '';
    if (hasEffectiveDates && effectiveStartDate && effectiveEndDate) {
      return `https://www.google.com/search?q=car+rental+in+${encodeURIComponent(destClean)}+from+${effectiveStartDate}+to+${effectiveEndDate}`;
    }
    return itinerary.budget?.transport?.deepLink || `https://www.google.com/search?q=car+rental+in+${encodeURIComponent(destClean)}`;
  })();
  ...
  return (
    <div id="itinerary-board" className="space-y-8 print:p-0">
      ...
      <div className="hidden print:block page-break-after border-b-4 border-slate-900 pb-8 mb-8">
        ...
        {/* AI Accommodation & Base Camp Strategy Section */}
        <div className="bg-gradient-to-r from-indigo-50/60 to-slate-50/60 rounded-2xl p-6 border border-indigo-150/80 shadow-3xs mb-8" id="ai-accom-recommendation">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-indigo-100/60">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-white rounded-xl text-indigo-700 font-bold shrink-0 shadow-3xs border border-indigo-105">
                <Home className="w-5 h-5 text-indigo-600" />
              </div>
              <div>
                <h4 className="font-extrabold text-sm text-slate-950 tracking-tight flex items-center gap-2">
                  🏡 AI Base Camp & Accommodation Recommendation
                </h4>
                <p className="text-[10px] uppercase font-bold text-indigo-600 tracking-wider">
                  System-recommended Hotel vs. Airbnb strategy & geo-rotation logistics
                </p>
              </div>
            </div>
            {itinerary.accommodationRecommendation?.stayType && (
              <div className="flex items-center gap-1.5 self-start sm:self-auto">
                <span className="text-[10px] font-black uppercase px-2.5 py-1 rounded-full border bg-slate-900 text-white shadow-2xs font-mono tracking-wide">
                  Style: {itinerary.accommodationRecommendation.stayType}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 space-y-4">
            <div>
              <h5 className="text-xs font-black text-slate-900 flex items-center gap-1.5 mb-1.5">
                📍 {itinerary.accommodationRecommendation?.heading || "Stay Logistics Strategy Details"}
              </h5>
              <div className="p-3 bg-white rounded-xl border border-indigo-100 text-[11px] font-semibold text-indigo-950 leading-relaxed shadow-3xs">
                {itinerary.accommodationRecommendation?.locationStrategy || "Single home base recommended."}
              </div>
            </div>

            <p className="text-xs text-slate-700 leading-relaxed font-normal">
              {itinerary.accommodationRecommendation?.detailedReasoning || "Your travel planner analyzed activity densities. Standard recommendations minimize backtracking and adjust base camps sequentially based on daylight activities and check-in timelines."}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* AI ACCOMMODATION & BASE CAMP STRATEGY SIDEBAR PANEL */}
          <div className="bg-indigo-50/50 border border-indigo-150/80 rounded-3xl p-6 space-y-4" id="ai-accom-sidebar-card">
            <h4 className="font-bold text-slate-900 flex items-center gap-2">
              <Home className="w-5 h-5 text-indigo-605" />
              Base Camp & Stay Plan
            </h4>
            <div className="space-y-3">
              <div className="text-xs">
                <span className="text-[10px] uppercase font-bold text-indigo-600 block mb-1">Accommodation Vibe</span>
                <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-950 text-white select-none">
                  {itinerary.accommodationRecommendation?.stayType || "Split Stay"} Style
                </span>
              </div>
              
              <div className="text-xs">
                <span className="text-[10px] uppercase font-bold text-indigo-600 block mb-1">Base Camp Map Strategy</span>
                <p className="p-3 bg-white border border-indigo-100/60 rounded-xl text-[11px] font-bold text-indigo-950 leading-relaxed shadow-3xs">
                  {itinerary.accommodationRecommendation?.locationStrategy || "Single base camp recommended."}
                </p>
              </div>

              <div className="text-xs text-slate-700 leading-relaxed">
                <span className="text-[10px] uppercase font-bold text-indigo-600 block mb-1">Why This Base Camp?</span>
                <p className="font-normal text-[11px]">
                  {itinerary.accommodationRecommendation?.detailedReasoning || "Your travel planner customizes stays to minimize transfers."}
                </p>
              </div>
            </div>
          </div>
